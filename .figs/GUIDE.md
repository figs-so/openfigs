...
- **Day to day:** `figs inbox` (start here — your humans' answers + your unfinished jobs) · `figs checkpoint` (a job opens / progresses) · `figs report` (its outcome) · `figs ask` (needs a human) · `figs resolve` (close an ask) — they stamp the bookkeeping and push themselves.
