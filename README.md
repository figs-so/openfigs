...
itself with the CLI's verbs — **`figs checkpoint`** (a job opens / progresses — it survives a
crash), **`figs report`** (its outcome, with attached artifacts), **`figs ask`** (anything that
needs a human), **`figs resolve`** (close an ask) — each pushes itself. (The CLI is open
source; the app is a hosted product.)
